...
class YTDLSource(discord.PCMVolumeTransformer):
	...
	async def create_source(cls, bot, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
		...
		if "http" not in search:
			...
			embed.set_footer(text=f"15초 안에 미선택시 **취소**댐!.")
			...
			await song_list_message.delete(delay = 15)
...
class Music(commands.Cog):
	...
	def cog_check(self, ctx: commands.Context):
		if not ctx.guild:
			raise commands.NoPrivateMessage('이 명령어는 개인채널에서 쓸수 없어오')
	...
	async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
		await ctx.send('에러ㅜㅜ : {}'.format(str(error)))
	'''
	...
	async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
		if not channel and not ctx.author.voice:
			raise VoiceError(':no_entry_sign: 음성채널 들어가바')
	...
	async def _leave(self, ctx: commands.Context):
		if not ctx.voice_state.voice:
			return await ctx.send(':no_entry_sign: 음성채널 들어가바')
	...
	async def _volume(self, ctx: commands.Context, *, volume: int):
		...
		if not 0 < volume < 101:
			return await ctx.send('```소리크기 1~100 사이 정수로 해주세오```')

		if vc.source:
			vc.source.volume = volume / 100

		ctx.voice_state.volume = volume / 100
		await ctx.send(':loud_sound: 소리 {}%로 바꿔써!'.format(volume))
	...
	async def _queue(self, ctx: commands.Context, *, page: int = 1):
		...
		if len(ctx.voice_state.songs) == 0:
			return await ctx.send(':mute: 재생목록이 빈!')
	...
	async def _loop(self, ctx: commands.Context):
		...
		if ctx.voice_state.loop :
			result = await ctx.send('반복 켜져따!')
		else:
			result = await ctx.send('반복 꺼져따.. 히잉')
		await result.add_reaction('🔁')
	...
	async def _play(self, ctx: commands.Context, *, search: str):
		...
		async with ctx.typing():
			try:
				...
				if not source:
					return await ctx.send(f"노래예약 취소")
			except YTDLError as e:
				await ctx.send('에러ㅜㅜ : {}'.format(str(e)))
			else:
				song = Song(source)
	...
	async def playText_(self, ctx):
		...
		#sayMessage = msg
		await MakeSound('흐갹! 잠시 후 사용해주세요.', './say' + str(ctx.guild.id))
		await ctx.send("```흐갹! 잠시 후 사용해주세요.```", tts=False)
